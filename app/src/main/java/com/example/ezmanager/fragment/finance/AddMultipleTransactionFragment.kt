...
class AddMultipleTransactionFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view=inflater.inflate(R.layout.fragment_add_multiple_transaction, container, false)
        ...
        val addMultipleTransaction:Button=view.findViewById(R.id.btnAddNewTransactionAndGo)
        val dateSetListener =
        ...
        addMultipleTransaction.setOnClickListener {
            if (newTitle.editText?.text.toString().trim().isNotEmpty() && newAmount.editText?.text.toString().trim().isNotEmpty() && transactionType!="F"){
                ...
                requireActivity().supportFragmentManager.beginTransaction().apply {
                    replace(R.id.content,sameFragment)
                }.commit()
            }
            else{
                if (newTitle.editText?.text.toString().trim().isEmpty())
